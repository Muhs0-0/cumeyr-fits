...
export default function EditVariantModal({ product, variant, onClose, onSave }: EditVariantModalProps) {
  ...
  useEffect(() => {
    // Load product's available sizes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch(`${API_BASE}/api/admin/variants/${variant.id}`, {
        method: "PATCH",
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[60]">
      <div className="bg-gray-900 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-green-500/20">
        <div className="bg-gradient-to-r from-green-600 to-green-500 p-6 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-white">Edit Variant</h2>
            <p className="text-green-100 text-sm">{product.name} - {variant.color}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="bg-gradient-to-r from-green-900/20 to-gray-800 rounded-lg p-4 border border-green-500/20">
            <p className="text-gray-300 text-sm">
              Edit this color variant's details including sizes, pricing, stock, and image.
            </p>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              />
              <label
                htmlFor="variant-image-upload-edit"
